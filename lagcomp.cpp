...
bool LagCompensation::StartPrediction(AimPlayer* data) {
	...
	int lat_ticks_compensated = g_cl.m_latency_ticks + 1; /*g_menu.main.antiaim.lag_shot.get() ? g_cl.m_latency_ticks + 1 : g_cl.m_latency_ticks; */

	if (lat_ticks_compensated <= lag) {
		if (lat_ticks_compensated > lag - updatedelta)
			data->m_delayed = true;
	...
	g_bones.SetupBones(record->m_player, nullptr, 128, 0x0007FF00 & ~0x00040000, record->m_sim_time, 2);
...
void LagCompensation::PredictAnimations(CCSGOPlayerAnimState* state, LagRecord* record) {
	...
	backup.m_origin = player->GetAbsOrigin();
	...
	bool bot = game::IsFakePlayer(record->m_player->index());
	...
	player->SetAbsOrigin(backup.m_origin);
}
