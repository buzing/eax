...
void Client::StartMove( CUserCmd* cmd ) {
	...
	m_max_lag = ( m_local->m_fFlags( ) & FL_ONGROUND ) ? g_menu.main.antiaim.lag_limit.get() : g_menu.main.antiaim.lag_limit.get()-1;
...
void Client::DoMove( ) {
	...
	if(!m_weapon_fire && m_weapon_type != WEAPONTYPE_GRENADE && m_weapon_type != WEAPONTYPE_C4 && g_menu.main.aimbot.auto_weapons.get())
...
void Client::UpdateInformation() {
	...
	g_bones.SetupBones(g_cl.m_local, g_cl.m_matrix, 128, 0x0007FF00, g_csgo.m_globals->m_curtime, 0);
