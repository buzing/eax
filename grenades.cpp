		...
		if (m_id == HEGRENADE) {
			if (render::WorldToScreen(prev, screen))
				if (g_menu.main.misc.esp_style.get() == 0) {
					...
					render::esp.string(screen.x, screen.y + 5, dumbo_dagadt, tfm::format(XOR("%i"), (int)target.first) + " dmg", render::ALIGN_CENTER);
				}
		}
	}
	...
	for (const auto& b : m_bounces) {
		...
		if (render::WorldToScreen(b.point, screen))
			if (g_menu.main.misc.esp_style.get() == 0) {
				...
				render::rect_filled(screen.x - 2, screen.y - 2, 4, 4, Color(25, 25, 25, 135));
				render::rect_filled(screen.x - 1, screen.y - 1, 2, 2, b.color);
				render::gradient2(screen.x - 2, screen.y - 2, 4, 4, Color(20, 20, 20, 0), Color(20, 20, 20, 150));
		}
	}
}
